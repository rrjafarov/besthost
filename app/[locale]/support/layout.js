...
//           </div>
//         </div>
//         <Footer servicesData={servicesData} t={t} contact={contact.data} />
//       </div>
//     </>
//   );
// };
...
const SupportPage = ({ children }) => {
  ...
  // Yeni: category ve servicesData state'leri (Header ve Footer için)
  ...
  useEffect(() => {
    ...
    // Yeni: category verisini çek
    async function fetchCategoryData() {
      try {
        const { data: categoryData } = await axiosInstance.get(`/page-data/categories`, {
          // headers: { Lang: lang.value },
          cache: "no-store",
        });
    ...
    // Yeni: services verisini çek (Footer'a göndermek için)
    async function fetchServicesData() {
      try {
        ...
        // Orijinal server-side kullanımında services.data.data olarak kullanıyorduk.
        // Footer muhtemelen array bekler; bu yüzden burada aynı yapıyı korudum.
        setServicesData(services?.data?.data ?? null);
      } catch (error) {
        console.error(error);
      }
    }
  ...
  // Bekleme: contact, t, category ve servicesData yüklendiğinde render et
  if (!contact || !t || !category || !servicesData) {
    return <div></div>;
  }
  ...
  return (
    <>
      <div className="background">
        {/* category artık state olarak mevcut ve Header'e prop olarak veriliyor */}
        <Header category={category} contact={contact.data} t={t} />
        <div className="besthostSupportPages">
          <div className="container">
            {/* Breadcrumbs */}
        ...
        {/* servicesData şimdi state'te mevcut, Footer'a iletiliyor */}
